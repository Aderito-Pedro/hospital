                                ...
                                <div class="">
                                    ...
                                    <form role="form" id="editPaymentForm" class="clearfix" action="finance/addPayment" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="col-md-5 row">
                                            ...
                                            <div class="col-md-12 payment pad_bot ocultar" id="id_tipo_pagamento">
                                                <label for="exampleInputEmail1">Escolha Forma de Pagamento</label>
                                                <select class="form-control m-bot15 js-example-basic-single pos_select" id="select_tipo_pagamento" name="value_pagamento_tipo" value=''>
...
<script>
    $(document).ready(function() {
        ...
        const pos_select = document.getElementById('pos_select');

        $('#pos_select').on('change', function() {
            const id_tipo_pagamento = document.getElementById('id_tipo_pagamento');
            id_tipo_pagamento.classList.add('ocultar');
            if (pos_select.value != 'add_new')
                verificaSeTemConvenio(pos_select.value);
        });

        function verificaSeTemConvenio($id_paciente) {
            $.ajax({
                url: "/hospital/finance/verificaSeConvenio?id=" + $id_paciente,
                type: 'get',
                success: function(data) {
                    if (data != "false") {
                        data = JSON.parse(data);
                        const id_tipo_pagamento = document.getElementById('id_tipo_pagamento');
                        id_tipo_pagamento.classList.remove('ocultar');
                        var htmls = '<option value="0" style="color: #41cac0 !important;">Particulat</option>';
                        for (var i = 0; i < data.length; i++) {
                            htmls += '<option value="'+data[i].id+'" style="color: #41cac0 !important;">'+data[i].descricao+' -- Saldo:'+data[i].valor+'kz</option>';
                        }
                        $('#select_tipo_pagamento').html(htmls);
...
<script src="common/js/js_finance.js"></script>
